...
% (the web address is given below):
%
...
%% Definitions and Import Related Functions
% If occlusion will be created set 'occlusionFlag' as 1.
occlusionFlag = 0;
% Set how many frames will be used to calculate threshold values. 
...
while hasFrame(video)
    Frames(:, :, :, frameNum) = im2double(readFrame(video));
    grayVideo(:, :, frameNum) = rgb2gray(Frames(:, :, :, frameNum));
    frameNum = frameNum + 1;
end 
...
] = targetPointer(Frames(:, :, :, 1), occlusionFlag);
...
[Foreground, Background] = automated_fastpcp(grayVideo);
...
% visualizeDetections(grayVideo, BinaryVideo_Closed);
...
% closest to the chosen one is regarded as the point which will be feeded 
% to the Kalman filter.
[feeding_points, DetectionVideo] = ...
featureExtracter       ...
(                      ...
BinaryVideo_Closed,    ...
Frames,                ...
...
% So as to visualize detection results, play 'DetectionVideo' by 
% uncommenting the code two rows below.
toc;
